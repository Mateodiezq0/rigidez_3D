...
class Barra:
    ...
    def matriz_r(self):
        ...
        # Columnas: x_local, y_local, z_local en coordenadas globales
    ...
    def matriz_R(self):
        r = self.matriz_r()
        R = np.zeros((12, 12))
        for i in range(4):  # 4 bloques de 3x3
    ...
    def resetear_reacciones(self): #ESTO LO AGREGUE YO
        ...
        self.reaccion_total_local = np.zeros(12)
        self.reac_eq_i_local = np.zeros(6)
        self.reac_eq_f_local = np.zeros(6)
    ...
    def añadirCarga(self, carga): #ESTO LO AGREGUE YO
        ...
        """
        f_emp = self.reacciones_carga_puntual(carga)
    ...
    def reacciones_carga_puntual(self, carga: "CargaPuntual"):
        ...
        # FLEXIÓN Y CORTANTES (en Zlocal y Ylocal)
        Qy = f_local[1]  # Fuerza Ylocal
        Qz = f_local[2]  # Fuerza Zlocal

        Qi_y = Qy * ((lj / self.L) ** 2) * (3 - 2 * (lj / self.L))
        Qj_y = Qy * ((li / self.L) ** 2) * (3 - 2 * (li / self.L))
        Mi_y = Qz * li * (lj / self.L) ** 2
        Mj_y = Qz * lj * (li / self.L) ** 2

        Qi_z = Qz * ((lj / self.L) ** 2) * (3 - 2 * (lj / self.L))
        Qj_z = Qz * ((li / self.L) ** 2) * (3 - 2 * (li / self.L))
        Mi_z = Qy * li * (lj / self.L) ** 2
        Mj_z = Qy * lj * (li / self.L) ** 2

        # 5. Vector de fuerzas nodales equivalentes (12) - tu convención
        f_emp = np.zeros(12)
        # Nodo inicial (i)
        f_emp[0] = -Ni        # Axial (X_local)
        f_emp[1] = -Qi_y      # Cortante (Y_local)
        f_emp[2] = -Qi_z      # Cortante (Z_local)
        f_emp[4] = -Mi_y      # Momento flexor en Y_local
        f_emp[5] = -Mi_z      # Momento flexor en Z_local
